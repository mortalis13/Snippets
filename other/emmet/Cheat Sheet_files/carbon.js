var _carbonads = {
	...
	{	
		...
		var serve = this.getServe(this.getUrlVar('serve'), placement);
...
function _carbonads_go (b) 
{
	...
	if(ad.fetch != null)
	{
		var fetch = document.createElement('script');
		    ...
		    fetch.src = ad.fetch + '/' + placement + '/8/';
		if(ad.click_redir != null)
			fetch.src += '?click_redir=' + encodeURIComponent(ad.click_redir) +'&r=' + time;
		else
			fetch.src += '/?' + time;
	...
	var el = document.createElement('span');
		el.innerHTML = '<span class="carbon-wrap"><a href="' + fulllink.replace('[timestamp]', time) + '" class="carbon-img" target="_blank"><img src="' + ad.image + '" alt="" border="0" height="100" width="130" style="max-width:130px;" /></a><a href="' + fulllink.replace('[timestamp]', time) + '" class="carbon-text" target="_blank">' + ad.description + '</a></span>';

	if(!_carbonads.isset(ad.removecarbon))
		el.innerHTML += '<a href="http://carbonads.net/" class="carbon-poweredby" target="_blank">ads via Carbon</a>';
